...
class Trainer(object):
	def __init__(self, corpus_dir):
		...
		with self.graph.as_default():
			tokenize_data = TokenizeData(corpus_dir=corpus_dir)

			self.hparams = tokenize_data.hparams
			self.train_batch = tokenize_data.get_training_batch()
			self.model = seq2seqModel(training = True, tokenize_data = tokenize_data, batch_input = self.train_batch)
...
if __name__ == "__main__":
    ...
    bt = Trainer(corpus_dir=corp_dir)
    bt.train(res_dir)